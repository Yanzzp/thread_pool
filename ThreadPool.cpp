...
pthread::ThreadPool::ThreadPool(int minNum, int maxNum) {
    // 实例化任务队列
...
void pthread::ThreadPool::addTask(Task task) {
    if (m_shutdown) {
        return;
    }
    // 添加任务，不需要加锁，任务队列中有锁
...
int pthread::ThreadPool::getAliveNumber() {
    int threadNum = 0;
...
int pthread::ThreadPool::getBusyNumber() {
    int busyNum = 0;
    pthread_mutex_lock(&m_lock);
    busyNum = m_busyNum;
    pthread_mutex_unlock(&m_lock);
...
// 工作线程任务函数
void *pthread::ThreadPool::worker(void *arg) {
...
// 管理者线程任务函数
void *pthread::ThreadPool::manager(void *arg) {
    ThreadPool *pool = static_cast<ThreadPool *>(arg);
    // 如果线程池没有关闭, 就一直检测
...
// 线程退出
void pthread::ThreadPool::threadExit() {
